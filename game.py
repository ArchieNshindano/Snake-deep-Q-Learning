...
class SnakeGameAI:
    ...
    def __init__(self):
        ...
        self.frame = 0
        self.state_history = deque(maxlen=50)  # storing the last 50 states
    ...
    def detect_loop(self):
        """
        Record the current state (snake head position and current direction)
        and check if this state has occurred frequently in the recent history.
        If it has, we consider that a loop.
        """
        # Define current state as a tuple of (head.x, head.y, current_direction)
        current_state = (self.snake.head.x, self.snake.head.y, self.snake.current_direction)
        self.state_history.append(current_state)
        # Count occurrences of current_state in the history.
        # Adjust the threshold as needed. Here, if the same state occurs more than 3 times,
        # we consider it as a loop.
        if list(self.state_history).count(current_state) > 3:
            return True
        return False
    ...
    def play_step(self, action):
        ...
        if self.snake.is_collided():  # or self.frame > 100 * len(self.snake.snake_elements):
            self.game_over = True
            self.reward -= 100
            return self._get_state(), self.reward, self.game_over, self.score
        ...
        if self.snake.head.x == self.apple.x and self.snake.head.y == self.apple.y:
            ...
            # Detect loops and penalize if detected
        if self.detect_loop():
                # Apply a penalty for looping behavior.
                self.reward -= 20
                # Optionally, you might consider ending the game if loops persist:
                # self.game_over = True
    ...
    def _get_state(self):
        ...
        distance = self.snake.get_distance_from_apple(self.apple)
